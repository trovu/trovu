...
wg berlin
```
This is like adding `site:en.wikipedia.org` to my Google search query. It will list only results from that domain, thus all Wikipedia articles that mention *berlin*.
...
```
Here, that first argument is typed again as *city*, so I can use a city name abbreviation:
```
gm hh, hauptstraße
```
#### Navigation
